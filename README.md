...
https://github.com/user-attachments/assets/3743c666-af23-44a6-9f43-b2ff483f57ec
...
「ESP32 勇者鬥惡龍」是一個簡單而有趣的遊戲，玩家需要控制勇者在螢幕上移動和跳躍，躲避惡龍吐的火球，並在適當時機與惡龍交戰以擊敗它。遊戲利用 ESP32 開發板實現，結合 OLED 顯示螢幕（顯示遊戲計時器）、蜂鳴器（提供音效）以及搖桿（控制角色移動）。玩家可以通過連接到 ESP32 的 Wi-Fi 熱點，並在瀏覽器中打開網頁界面來進行遊戲。
...
- **OLED 顯示**：在 SSD1306 顯示螢幕上顯示遊戲referrals
...
- **OLED 顯示螢幕**：SSD1306，128x64 像素，I2C 介面。
...
3. **連接硬體**：
   - **OLED 顯示螢幕**：SCL 連接至 GPIO 22，SDA 連接至 GPIO 21。
   - **蜂鳴器**：連接至 GPIO 14。
   - **搖桿**：
     - X 軸連接至 GPIO 35（ADC）。
     - Y 軸連接至 GPIO 32（ADC）。
...
5. **計時器**：
   - OLED 顯示螢幕將顯示遊戲持續時間（分:秒格式）。

---

## 難度

|關卡難度|火球數量上限(個)|火球生成間隔(毫秒)|火球高度偏移(像素)|火球速度(像素)|
|:---:|:---:|:---:|:---:|:---:|
|第一關|3|1182~3330|0|12|
|第二關|5|1059~2775|0|13|
|第三關|6|936~2220|0|14|
|第四關|7|813~1665|-30~25|15|
|第五關|7|690~1110|-60~25|16|
